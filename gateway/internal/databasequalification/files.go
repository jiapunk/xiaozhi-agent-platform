...
func WriteNew(path string, payload []byte) error {
	if len(payload) == 0 || len(payload) > int(maximumDocumentBytes) {
		return fmt.Errorf("database qualification output size is invalid")
	}
	file, err := os.OpenFile(path, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0o444)
	if err != nil {
		return fmt.Errorf("refusing to overwrite database qualification output: %w", err)
	}
	success := false
	defer func() {
		_ = file.Close()
	...
	if written, err := file.Write(payload); err != nil || written != len(payload) {
		return fmt.Errorf("write database qualification output")
	}
	if err := file.Sync(); err != nil {
		return fmt.Errorf("sync database qualification output")
	}
	if err := file.Close(); err != nil {
		return fmt.Errorf("close database qualification output")
	}
	success = true
	return nil
}
