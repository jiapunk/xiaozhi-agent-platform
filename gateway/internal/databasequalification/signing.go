...
func verifyDomainPayload(domain, payload []byte, signature,
	...
	if err != nil || len(decoded) != ed25519.SignatureSize ||
		...
		return fmt.Errorf("managed database qualification signature is invalid")
	}
...
func loadEd25519PrivateKey(path string) (ed25519.PrivateKey, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("managed database private key rejected")
	}
	block, rest := pem.Decode(payload)
	if block == nil || block.Type != "PRIVATE KEY" || len(bytes.TrimSpace(rest)) != 0 {
		return nil, fmt.Errorf("managed database private key is invalid")
	}
...
func loadEd25519PublicKey(path string) (ed25519.PublicKey, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("managed database public key rejected")
	}
	block, rest := pem.Decode(payload)
	if block == nil || block.Type != "PUBLIC KEY" || len(bytes.TrimSpace(rest)) != 0 {
		return nil, fmt.Errorf("managed database public key is invalid")
	}
