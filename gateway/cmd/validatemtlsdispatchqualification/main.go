...
func main() {
	receiptPath := flag.String("receipt", "", "signed M71 receipt")
	receiptPublicKey := flag.String("trusted-public-key", "", "trusted M71 public key")
	receiptKeyID := flag.String("expected-signing-key-id", "", "expected M71 signing key ID")
	...
	if flag.NArg() != 0 || *receiptPath == "" || *receiptPublicKey == "" ||
		*receiptKeyID == "" || *observationPath == "" || *appReceiptPath == "" ||
		...
		*ociReleaseID == "" || *evaluationTimeText == "" {
		fail("all mTLS dispatch qualification trust inputs are required")
	}
	...
	observationDigest := mustDigest(*observationPath,
		"mTLS dispatch observation digest unavailable")
	appReceiptDigest := mustDigest(*appReceiptPath,
		"App delivery receipt digest unavailable")
	providerDigest := mustDigest(*providerReceiptPath,
		"provider receipt digest unavailable")
	appObservationDigest := mustDigest(*appObservationPath,
		"App observation digest unavailable")
	appAttestationDigest := mustDigest(*appAttestationPath,
		"App attestation digest unavailable")
	deploymentAttestationDigest := mustDigest(*deploymentAttestationPath,
		"deployment attestation digest unavailable")
	verified, err := mtlsdispatchqualification.VerifyReceiptFile(
		*receiptPath, mtlsdispatchqualification.ReceiptVerifyOptions{
			TrustedPublicKey:                    *receiptPublicKey,
			ExpectedSigningKeyID:                *receiptKeyID,
			ExpectedQualificationID:             *qualificationID,
			ExpectedEnvironment:                 *environment,
			ExpectedDeploymentID:                *deploymentID,
			ExpectedOCIReleaseID:                *ociReleaseID,
			ExpectedObservationSHA256:           observationDigest,
			ExpectedAppDeliveryReceiptSHA256:    appReceiptDigest,
			ExpectedDeploymentAttestationSHA256: deploymentAttestationDigest,
			ExpectedEvaluationTime:              *evaluationTimeText,
			RequireLive:                         *requireLive})
	if err != nil {
		fail("mTLS dispatch qualification receipt rejected")
	}
	expected, err := mtlsdispatchqualification.BuildReceipt(
	...
	if err != nil || !mtlsdispatchqualification.ReceiptMatchesEvidence(
		verified, expected) {
		fail("mTLS dispatch qualification evidence bundle rejected")
	}
